...
static int64_t ram_total = -1;
static int64_t swap_total = -1;
// State for computing CPU usage.
static unsigned int64_t cpu_prev_user[NUM_CPU_ENTRIES],
    cpu_prev_userNice[NUM_CPU_ENTRIES];
static unsigned int64_t cpu_prev_system[NUM_CPU_ENTRIES],
    cpu_prev_idle[NUM_CPU_ENTRIES];
static unsigned int64_t cpu_prev_ioWait[NUM_CPU_ENTRIES],
    cpu_prev_irq[NUM_CPU_ENTRIES], cpu_prev_irqSoft[NUM_CPU_ENTRIES];
static double cpu_percents[NUM_CPU_ENTRIES];
static FILE *cpu_proc_stat_file;
// State for limiting log file sizes.
static int64_t last_logrotate_time_us = 0;
// The main process ID of the program.
static int cetiApp_pid = -1;
// Thread IDs, which will be updated by the relevant threads if they are
// enabled.
...
int init_systemMonitor() {
  // Get initial readings from /proc/stat, so differences can be taken later to
  // compute CPU usage.
  update_cpu_usage();
  ...
  if (init_data_file(
          systemMonitor_data_file, SYSTEMMONITOR_DATA_FILEPATH,
...
void *systemMonitor_thread(void *paramPtr) {
  ...
  // Set the previous time such that it will print once at most 30s after
  // starting and thereafter according to the desired period.
  int64_t last_tid_print_time_us =
      get_global_time_us() +
      (TID_PRINT_PERIOD_US > 30000000 ? (30000000 - TID_PRINT_PERIOD_US) : 0);
#endif
  while (!g_exit) {
// Print the thread IDs if desired
    ...
    if (get_global_time_us() - last_tid_print_time_us >= TID_PRINT_PERIOD_US) {
      ...
      CETI_LOG(" %6d: pressureTemperature_thread",
               g_pressureTemperature_thread_tid);
    ...
    if (!g_stopAcquisition) {
      // Acquire system information as close as possible to the above
      // timestamps.
      ...
      if (systemMonitor_data_file == NULL) {
        ...
        for (int cpu_entry_index = 0; cpu_entry_index < NUM_CPU_ENTRIES;
             cpu_entry_index++)
          fprintf(systemMonitor_data_file, ",%0.2f",
                  cpu_percents[cpu_entry_index]);
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_audio_thread_spi_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_audio_thread_writeData_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_ecg_thread_getData_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_ecg_thread_writeData_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_imu_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_light_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_pressureTemperature_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_boardTemperature_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_battery_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_recovery_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_stateMachine_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_command_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_rtc_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_systemMonitor_thread_tid));
        fprintf(systemMonitor_data_file, ",%d",
                get_cpu_id_for_tid(g_goPros_thread_tid));
        fprintf(systemMonitor_data_file, ",%lld", ram_free);
        fprintf(systemMonitor_data_file, ",%0.2f",
                100.0 * ((double)ram_free) / ((double)ram_total));
        fprintf(systemMonitor_data_file, ",%lld", swap_free);
        fprintf(systemMonitor_data_file, ",%0.2f",
                100.0 * ((double)swap_free) / ((double)swap_total));
...
//  but the logs are generally small (one test indicated about 1.1 MiB over 11
//  hours, which would fill the 1 GiB free space threshold in about 1.2 years).
...
int64_t get_swap_total() {
  sysinfo(&memInfo);
  int64_t totalSwap = memInfo.totalswap;
...
int64_t get_swap_free() {
  sysinfo(&memInfo);
  int64_t swapFree = memInfo.freeswap;
  // Multiply in next statement to avoid int overflow on right hand side...
...
int64_t get_ram_free() {
  sysinfo(&memInfo);
  int64_t physMemFree = memInfo.freeram;
  // Multiply in next statement to avoid int overflow on right hand side...
...
int update_cpu_usage() {
  unsigned int64_t cpu_user, cpu_userNice, cpu_system, cpu_idle;
  unsigned int64_t cpu_ioWait, cpu_irq, cpu_irqSoft;
  unsigned int64_t cpu_total;
  ...
  for (int cpu_entry_index = 0; cpu_entry_index < NUM_CPU_ENTRIES;
    ...
    if (num_read >= 0) {
      ...
      if (res < 7) {
        ...
        cpu_total = (cpu_user - cpu_prev_user[cpu_entry_index]) +
                    (cpu_userNice - cpu_prev_userNice[cpu_entry_index]) +
                    (cpu_system - cpu_prev_system[cpu_entry_index]);
        cpu_percents[cpu_entry_index] = cpu_total;
        cpu_total += (cpu_idle - cpu_prev_idle[cpu_entry_index]) +
                     (cpu_ioWait - cpu_prev_ioWait[cpu_entry_index]) +
                     (cpu_irq - cpu_prev_irq[cpu_entry_index]) +
                     (cpu_irqSoft - cpu_prev_irqSoft[cpu_entry_index]);
        cpu_percents[cpu_entry_index] /= cpu_total;
...
// NOTE: Previously tried calling sched_getcpu() and getcpu() from within each
// thread,
