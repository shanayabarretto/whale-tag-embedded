...
num_goPros = len(
    goPro_ids
)  # note that this can be greater than the number in the C program (they will simply not be started)
...
async def control_goPro(goPro_index, command):
    ...
    if command == goPro_commands["start"] and goPro_status[goPro_index] == 1:
        logger.info(
            "GoPro %d (ID %d) is already running; ignoring start command"
            % (goPro_index, goPro_id)
        )
        print(
            "GoPro %d (ID %d) is already running; ignoring start command"
            % (goPro_index, goPro_id)
    ...
    if command == goPro_commands["stop"] and goPro_status[goPro_index] == 0:
        logger.info(
            "GoPro %d (ID %d) is already stopped; ignoring stop command"
            % (goPro_index, goPro_id)
        )
        print(
            "GoPro %d (ID %d) is already stopped; ignoring stop command"
    ...
    def notification_handler(handle: int, data: bytes) -> None:
        ...
        if (
            client.services.characteristics[handle].uuid == response_uuid
            and data[2] == 0x00
        ):
    ...
    try:
        client = await asyncio.wait_for(
            connect_ble(notification_handler, goPro_id), timeout=20
        )
        event.clear()
    except (
        asyncio.TimeoutError,  # The device was probably not found in the Bluetooth scan
        RuntimeError,
    ):  # Probably exceeded the 10 retry attempts to connect
        event.clear()
        logger.info(
            "Error or timeout connecting to GoPro %d (ID %d)" % (goPro_index, goPro_id)
        )
        print(
            "Error or timeout connecting to GoPro %d (ID %d)" % (goPro_index, goPro_id)
        )
    ...
    # Start the GoPro.
    if (
        command == goPro_commands["start"]
    ):  # Write to command request BleUUID to turn the shutter on
        logger.info(
            "Setting the shutter on for GoPro %d (ID %d)" % (goPro_index, goPro_id)
        )
        print("Starting GoPro %d (ID %d)" % (goPro_index, goPro_id))
        await client.write_gatt_char(
            COMMAND_REQ_UUID, bytearray([3, 1, 1, 1]), response=True
        )
    ...
    # Stop the GoPro.
        ...
        logger.info(
            "Setting the shutter off for GoPro %d (ID %d)" % (goPro_index, goPro_id)
        )
        print("Stopping GoPro %d (ID %d)" % (goPro_index, goPro_id))
        await client.write_gatt_char(
            COMMAND_REQ_UUID, bytearray([3, 1, 1, 0]), response=True
        )
...
async def main() -> None:
    ...
    while time.time() - last_message_time_s < 2 * connection_heartbeat_period_s:
        ...
        try:
            data_is_available = select.select(
                [c_socket], [], [], socket_receive_timeout_s
            )[0]
            if data_is_available:
                ...
                if len(buff) > 0:
                    ...
                    print(
                        "Received command %d for goPro index %d"
                        % (command, goPro_index)
                    )
                    # Process the command.
                    ...
                    if command != goPro_commands["heartbeat"]:
                        ...
                        c_message = GoPro_C_Message(
                            goPro_index, command, 1, 1 if success else 0
                        )
